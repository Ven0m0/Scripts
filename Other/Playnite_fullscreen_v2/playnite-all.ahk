...
if (mode = "fullscreen") {
    ...
    if (mode = "tv_sound") {
        svv := FindExe("SoundVolumeView.exe", [A_ScriptDir . "\SoundVolumeView\SoundVolumeView.exe"])
        if (svv = "")
            MsgBox("SoundVolumeView.exe not found; audio switch will be skipped.")
    }
    ...
    if (mode = "tv_sound" and svv != "") {
        ; Switch audio to TV; adjust device name as needed
...
CheckPlaynite() {
    ...
    if (mode = "tv" or mode = "tv_firefox" or mode = "tv_sound") {
        ...
        if (mode = "tv_sound" and svv != "") {
            ; Restore to generic speakers/headphones; adjust name if needed
            RunWait(svv . ' /SetDefault "Speakers"', , "hide")
        }
